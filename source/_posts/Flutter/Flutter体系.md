...
- flutter 根目录
  - packages：
    ...
    - flutter_test:
    - flutter_tools:创建、构建、测试、编译Flutter工具
    - flutter_web_plugins:
    - fuchsia_remote_debug_protocol:
    - integration_test:
  - bin:
    - internal
      - engine.version：指定Flutter使用的引擎版本，文件中包含的是Flutter引擎仓库的一个提交哈希值
      - engin.merge_method：控制引擎自动滚动器创建的拉取请求的合并方式。如果是 squash，那么无论拉取请求中有多少个引擎提交，都只会有一个提交。如果是 rebase，那么框架中的提交数量将等于拉取请求中的引擎提交数量。
      - flutter_plugin.version：这个文件指定了用于测试的 flutter/plugins 仓库的版本。flutter/plugins 仓库并不是 flutter/flutter 的上游依赖，它只是测试套件的一部分，用于验证，而这里固定的版本确保了每个 flutter/flutter 提交的测试都是确定的。 
      - shared.sh：共享的脚本
